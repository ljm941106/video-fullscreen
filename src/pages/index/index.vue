  ...
  <div class="container index">
    ...
    <swiper class="banner" indicator-dots="true" indicator-color="rgba(0, 0, 0, .3)" indicator-active-color="rgba(0, 0, 0, .88)" autoplay="true" interval="5000" duration="500">
      <block v-for="i in banner" :key="index">
        <swiper-item @click="bannerCheck(i.video_id)">
          <image :src="i.img" class="slide-image" width="355" height="150" />
        </swiper-item>
      </block>
    </swiper>
    ...
    methods: {
      ...
      bannerCheck(id) {
        fly.get('get_info', {
          ...
          if(item) {
            wx.navigateTo({
              url: '/pages/item/main?pid=' + item.id + '&cid=' + item.class_id
            })
          }
        })
      },
